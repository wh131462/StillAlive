...
export default function PasswordVaultScreen() {
  ...
  const [showForgottenDelete, setShowForgottenDelete] = useState(false);
  ...
  return <SafeAreaView style={styles.safeArea}>
    ...
    <ScrollView contentContainerStyle={styles.authContent} keyboardShouldPersistTaps="handled" showsVerticalScrollIndicator={false}>
      <View style={styles.lockIllustration}><View style={styles.lockRing}><SymbolView name={{ android: 'lock', ios: 'lock.fill', web: 'lock' }} size={34} tintColor={colors.life} type="hierarchical" /></View><View style={styles.lockLine} /></View>
      <Text style={styles.authEyebrow}>{creating ? 'CREATE PASSWORD VAULT' : 'UNLOCK PASSWORD VAULT'}</Text>
      <Text style={styles.authTitle}>{creating ? '创建你的密码本' : '解锁密码本'}</Text>
      <Text style={styles.authText}>{creating ? '主密码至少 6 个字符，只由你记住。忘记后无法找回，应用也不会保存它。' : '输入创建密码本时设置的主密码。输入内容不会被裁剪或改变大小写。'}</Text>
      <View style={styles.authForm}>
        <Text style={styles.fieldLabel}>主密码</Text><TextInput accessibilityLabel="主密码" autoCapitalize="none" autoCorrect={false} editable={!busy} importantForAutofill="no" onChangeText={setMasterPassword} placeholder="至少 6 个字符" placeholderTextColor={colors.inkFaint} secureTextEntry style={styles.input} textContentType="none" value={masterPassword} />
        {creating ? <><Text style={styles.fieldLabel}>再次输入</Text><TextInput accessibilityLabel="再次输入主密码" autoCapitalize="none" autoCorrect={false} editable={!busy} importantForAutofill="no" onChangeText={setConfirmation} placeholder="完整重复一次" placeholderTextColor={colors.inkFaint} secureTextEntry style={styles.input} textContentType="none" value={confirmation} /></> : null}
        <Pressable accessibilityRole="button" disabled={busy} onPress={() => void run(() => creating ? vault.create(masterPassword, confirmation) : vault.unlock(masterPassword))} style={({ pressed }) => [styles.primaryButton, busy && styles.disabled, pressed && styles.pressed]}><Text style={styles.primaryButtonText}>{busy ? '正在处理…' : creating ? '创建密码本' : '用主密码解锁'}</Text><SymbolView name={{ android: 'arrow_forward', ios: 'arrow.right', web: 'arrow_forward' }} size={18} tintColor={colors.onLife} type="hierarchical" /></Pressable>
      ...
      {!creating ? <Pressable onPress={() => setShowForgottenDelete((value) => !value)} style={styles.forgotButton}><Text style={styles.forgotText}>忘记主密码？</Text></Pressable> : null}
      {showForgottenDelete ? <View style={styles.forgotCard}><Text style={styles.forgotTitle}>主密码无法重置</Text><Text style={styles.forgotBody}>只能永久删除密码本并重新创建。日记、人物和媒体不会受到影响。输入“永久删除密码本”后执行。</Text><TextInput autoCapitalize="none" autoCorrect={false} onChangeText={setDeleteConfirmation} placeholder="永久删除密码本" placeholderTextColor={colors.inkFaint} style={styles.dangerInput} value={deleteConfirmation} /><Pressable disabled={busy || deleteConfirmation !== '永久删除密码本'} onPress={() => void run(async () => { await vault.forceDeleteVault(); setShowForgottenDelete(false); setDeleteConfirmation(''); })} style={[styles.dangerButton, (busy || deleteConfirmation !== '永久删除密码本') && styles.disabled]}><Text style={styles.dangerButtonText}>永久删除密码本</Text></Pressable></View> : null}
      <View style={styles.localNotice}><SymbolView name={{ android: 'cloud_off', ios: 'icloud.slash', web: 'cloud_off' }} size={17} tintColor={colors.inkFaint} type="hierarchical" /><Text style={styles.localNoticeText}>不联网 · 不同步 · 主密码不落盘</Text></View>
    </ScrollView>
  </SafeAreaView>;
}

function VaultHeader({ onBack, right }: { onBack(): void; right?: ReactNode }) {
  return <View style={styles.header}><Pressable accessibilityLabel="返回" accessibilityRole="button" onPress={onBack} style={styles.headerButton}><SymbolView name={{ android: 'chevron_left', ios: 'chevron.left', web: 'chevron_left' }} size={22} tintColor={colors.inkSoft} type="hierarchical" /></Pressable><Text style={styles.headerTitle}>密码本</Text><View style={styles.headerRight}>{right}</View></View>;
}
...
const styles = createThemedStyles(() => ({
  safeArea: { flex: 1, backgroundColor: colors.paper }, loading: { flex: 1, alignItems: 'center', justifyContent: 'center' }, loadingMark: { color: colors.life, fontFamily: typography.display, fontSize: 30 }, loadingText: { marginTop: spacing.sm, color: colors.inkFaint, fontSize: typography.size.caption },
  header: { minHeight: 56, paddingHorizontal: spacing.md, flexDirection: 'row', alignItems: 'center', borderBottomWidth: StyleSheet.hairlineWidth, borderBottomColor: colors.line }, headerButton: { width: 44, height: 44, alignItems: 'center', justifyContent: 'center' }, headerTitle: { flex: 1, color: colors.ink, fontFamily: typography.display, fontSize: 18, textAlign: 'center' }, headerRight: { width: 88, alignItems: 'flex-end' }, headerActions: { flexDirection: 'row' },
  ...
  authContent: { padding: spacing.lg, paddingBottom: spacing.xxl }, lockIllustration: { height: 112, alignItems: 'center' }, lockRing: { width: 84, height: 84, alignItems: 'center', justifyContent: 'center', borderWidth: 1, borderColor: colors.lifeLine, borderRadius: 42, backgroundColor: colors.sheet }, lockLine: { width: 1, height: 28, backgroundColor: colors.lifeLine }, authEyebrow: { marginTop: spacing.md, color: colors.life, fontFamily: typography.mono, fontSize: typography.size.meta, letterSpacing: 1.4, textAlign: 'center' }, authTitle: { marginTop: spacing.sm, color: colors.ink, fontFamily: typography.display, fontSize: 29, lineHeight: 38, textAlign: 'center' }, authText: { marginTop: spacing.md, color: colors.inkSoft, fontSize: typography.size.caption, lineHeight: 19, textAlign: 'center' }, authForm: { marginTop: spacing.xl, padding: spacing.lg, borderTopRightRadius: radius.xl, borderBottomLeftRadius: radius.xl, backgroundColor: colors.sheet }, fieldLabel: { marginTop: spacing.md, marginBottom: 6, color: colors.inkSoft, fontSize: typography.size.meta, fontWeight: '700' }, input: { minHeight: 50, paddingHorizontal: spacing.md, borderWidth: StyleSheet.hairlineWidth, borderColor: colors.line, borderRadius: radius.md, color: colors.ink, backgroundColor: colors.paper, fontSize: typography.size.body }, primaryButton: { minHeight: 52, marginTop: spacing.lg, paddingHorizontal: spacing.lg, flexDirection: 'row', gap: spacing.sm, alignItems: 'center', justifyContent: 'center', borderRadius: radius.md, backgroundColor: colors.life }, primaryButtonText: { color: colors.onLife, fontSize: typography.size.caption, fontWeight: '800' }, secondaryButton: { minHeight: 50, marginTop: spacing.md, flexDirection: 'row', gap: spacing.sm, alignItems: 'center', justifyContent: 'center', borderWidth: 1, borderColor: colors.lifeLine, borderRadius: radius.md }, secondaryButtonText: { color: colors.life, fontSize: typography.size.caption, fontWeight: '700' },
  forgotButton: { minHeight: 44, alignItems: 'center', justifyContent: 'center' }, forgotText: { color: colors.inkFaint, fontSize: typography.size.caption, textDecorationLine: 'underline' }, forgotCard: { marginTop: spacing.md, padding: spacing.lg, borderWidth: StyleSheet.hairlineWidth, borderColor: colors.dangerLine, borderRadius: radius.lg, backgroundColor: colors.dangerLight }, forgotTitle: { color: colors.danger, fontSize: typography.size.body, fontWeight: '800' }, forgotBody: { marginTop: spacing.sm, color: colors.inkSoft, fontSize: typography.size.caption, lineHeight: 19 }, dangerInput: { minHeight: 48, marginTop: spacing.md, paddingHorizontal: spacing.md, borderWidth: 1, borderColor: colors.dangerLine, borderRadius: radius.md, color: colors.ink, backgroundColor: colors.paper }, dangerButton: { minHeight: 48, marginTop: spacing.md, alignItems: 'center', justifyContent: 'center', borderRadius: radius.md, backgroundColor: colors.danger }, dangerButtonText: { color: colors.onLife, fontSize: typography.size.caption, fontWeight: '800' }, localNotice: { marginTop: spacing.xl, flexDirection: 'row', gap: spacing.sm, alignItems: 'center', justifyContent: 'center' }, localNoticeText: { color: colors.inkFaint, fontFamily: typography.mono, fontSize: typography.size.meta }, disabled: { opacity: 0.4 }, pressed: { opacity: 0.72 },
}));
