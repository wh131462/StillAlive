import type { AlbumMedia, AppThemeId, BirthdayCalendar, BirthdayNotificationSchedule, BirthdayReminderMode, Book, BookExcerpt, BookFormat, BookLocationType, BookParseStatus, CheckIn, Draft, Gender, Media, MusicCollectionEntry, MusicPlaybackMode, MusicPlaylist, MusicPlaylistEntry, MusicTrack, NameStyleId, Person, PersonAlbum, PersonTagAssignment, Post, ProfileCollectionField, ProfileCollectionRequestStatus, ReaderTocItem, ReadingNoteSource, TagDefinition, TagGroup, TagSystemSetting } from '@still-alive/types';
...
export interface MediaRow { id: string; local_path: string; mime_type: string; width: number | null; height: number | null; checksum: string; created_at: string; kind?: 'image' | 'audio' | 'book' | null; original_name?: string | null; size_bytes?: number | null; }
export interface MusicTrackRow { id: string; media_id: string; cover_media_id: string | null; title: string; artist: string | null; album: string | null; duration_ms: number | null; created_at: string; updated_at: string; }
...
export interface BackupSnapshot {
  ...
  albums?: PersonAlbum[]; albumMedia?: AlbumMedia[]; musicTracks?: MusicTrack[]; musicCollectionEntries?: MusicCollectionEntry[];
