...
export default function BackupScreen() {
  ...
  const confirmVaultRestore = (parsed: ParsedBackup) => {
    ...
    feedback.alert(hasLocalVault ? '替换当前密码本？' : '恢复备份中的密码本？', hasLocalVault ? '当前密码本会被备份中的加密密码本替换。日记和媒体不受这一步影响。' : '将把备份中的加密密码本恢复到本机。生物识别快捷解锁不会随备份恢复。', [
      { text: '取消', style: 'cancel' },
  ...
  const confirmDeleteAll = () => {
    feedback.alert('删除这台设备上的全部内容？', '日记、草稿、人物、媒体、密码本和设置都会被真实删除。之前导出的备份文件不会被删除。', [
      { text: '取消', style: 'cancel' },
