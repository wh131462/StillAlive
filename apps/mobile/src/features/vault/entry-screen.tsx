...
export default function PasswordVaultEntryScreen() {
  ...
  const save = async () => {
    try {
      ...
      setPassword('');
      router.back();
    } catch (cause: unknown) { feedback.alert('保存失败', errorMessage(cause)); }
  ...
  const copy = async () => {
    try {
      ...
      setCopied(true);
      if (copyFeedbackTimeoutRef.current) clearTimeout(copyFeedbackTimeoutRef.current);
      copyFeedbackTimeoutRef.current = setTimeout(() => { setCopied(false); copyFeedbackTimeoutRef.current = null; }, 2_000);
    } catch (cause: unknown) { feedback.alert('复制失败', errorMessage(cause)); }
  };

  const confirmDelete = () => feedback.alert('删除这条密码记录？', '删除后无法恢复。', [
  ...
  const deleteEntry = async () => {
    ...
    catch (cause: unknown) { feedback.alert('删除失败', errorMessage(cause)); }
    finally { setBusy(false); }
  };
      ...
      <ScrollView contentContainerStyle={styles.content} keyboardShouldPersistTaps="handled" showsVerticalScrollIndicator={false}>
        ...
        <View style={styles.formCard}>
          ...
          <Field label="密码（必填）">
            ...
            {entry ? <View style={styles.passwordActions}><Pressable accessibilityLabel={copied ? '密码已复制' : '复制密码'} disabled={!password} onPress={() => void copy()} style={[styles.copyButton, !password && styles.disabled]}><SymbolView name={copied ? { android: 'check', ios: 'checkmark', web: 'check' } : { android: 'content_copy', ios: 'doc.on.doc', web: 'content_copy' }} size={16} tintColor={colors.life} type="hierarchical" /><Text accessibilityLiveRegion="polite" style={styles.copyText}>{copied ? '已复制' : '复制密码'}</Text></Pressable></View> : null}
          </Field>
