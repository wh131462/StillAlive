...
export default function PasswordVaultSettingsScreen() {
  ...
  const toggleBiometrics = async () => {
    ...
    catch (cause: unknown) { feedback.alert('快捷解锁设置失败', errorMessage(cause)); }
  ...
  const changePassword = async () => {
    try {
      ...
      feedback.alert('主密码已修改', '数据密钥已使用新主密码重新保护。请记住新主密码。');
    } catch (cause: unknown) { feedback.alert('修改失败', errorMessage(cause)); }
    finally { setBusy(null); }
  };
  ...
  return <SafeAreaView style={styles.safeArea}>
    <ToolPageHeader backAccessibilityLabel="返回密码本" backDisabled={busy !== null} onBack={() => router.back()} title="密码本安全" />
