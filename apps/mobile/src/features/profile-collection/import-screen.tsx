...
import { readProfileCollectionPrivateKey } from './profile-collection-key-storage';
import { decodeProfileCollectionResponseCode, parseProfileCollectionResponseInput, PROFILE_COLLECTION_PLAINTEXT_MAX_BYTES, validateProfileCollectionPayload } from './profile-collection';
import type { ProfileCollectionResponseEnvelopeV1, ProfileCollectionResponsePayloadV1 } from './profile-collection';
...
export default function ProfileCollectionImportScreen() {
  ...
  const confirmImport = async () => {
    ...
    try {
      ...
      feedback.alert('已保存', `确认的内容已更新到 ${person.name}。`, [{ text: '查看人物', onPress: () => router.replace({ pathname: '/person/[id]', params: { id: person.id } }) }]);
    } catch (cause) {
...
const styles = createThemedStyles(() => ({
  ...
  intro: { padding: spacing.lg, borderRadius: 6, backgroundColor: colors.sheet },
  ...
  input: { minHeight: 150, marginTop: spacing.md, padding: spacing.md, borderWidth: StyleSheet.hairlineWidth, borderColor: colors.lineSoft, borderRadius: 6, backgroundColor: colors.sheet, color: colors.ink, fontSize: 12, lineHeight: 19, textAlignVertical: 'top' },
  primaryButton: { minHeight: 54, marginTop: spacing.lg, alignItems: 'center', justifyContent: 'center', borderRadius: 6, backgroundColor: colors.life },
  primaryButtonDisabled: { opacity: 0.42 },
  primaryButtonText: { color: colors.onLife, fontSize: 12, fontWeight: '700' },
  errorCard: { marginTop: spacing.md, padding: spacing.md, borderRadius: 6, backgroundColor: colors.dangerLight },
  ...
  reviewHead: { padding: spacing.lg, borderRadius: 6, backgroundColor: colors.life },
  ...
  differenceList: { marginTop: spacing.lg, paddingHorizontal: spacing.md, overflow: 'hidden', borderRadius: 6, backgroundColor: colors.sheet },
  ...
  sameCard: { marginTop: spacing.lg, padding: spacing.lg, borderRadius: 6, backgroundColor: colors.sheet },
