...
export default function TagManagementScreen() {
  ...
  const confirmDeleteTag = async (tagId: string, name: string) => {
    ...
    feedback.alert(`删除“${name}”？`, count ? `该标签已用于 ${count} 个人物，删除后会解除关联。` : '删除后无法恢复。', [{ text: '取消', style: 'cancel' }, { text: '删除', style: 'destructive', onPress: () => void deleteTag(tagId).catch(showError) }]);
  };
  const confirmDeleteGroup = async (group: TagGroup) => {
    ...
    feedback.alert(`删除标签组“${group.name}”？`, count ? `组内选项存在 ${count} 条人物关联，删除后会全部解除。` : '组内选项也会一并删除。', [{ text: '取消', style: 'cancel' }, { text: '删除标签组', style: 'destructive', onPress: () => void deleteTagGroup(group.id).catch(showError) }]);
  };
  const saveEditing = () => {
...
const styles = createThemedStyles(() => ({
  ...
  typeGuide: { minHeight: 76, padding: spacing.md, flexDirection: 'row', alignItems: 'center', borderRadius: 6, backgroundColor: colors.lifeLight }, typeItem: { flex: 1, flexDirection: 'row', alignItems: 'center' }, typeIcon: { width: 36, height: 36, marginRight: spacing.sm, alignItems: 'center', justifyContent: 'center', borderRadius: 18, backgroundColor: colors.paper }, typeTitle: { color: colors.ink, fontSize: 11, fontWeight: '700' }, typeHint: { marginTop: 4, color: colors.inkFaint, fontSize: 8 }, typeDivider: { width: StyleSheet.hairlineWidth, height: 36, marginHorizontal: spacing.sm, backgroundColor: colors.line },
  ...
  groupCard: { marginTop: spacing.md, padding: spacing.md, borderRadius: 6, backgroundColor: colors.sheet }, groupHeader: { minHeight: 44, flexDirection: 'row', alignItems: 'center' }, groupName: { color: colors.ink, fontFamily: typography.display, fontSize: 17 }, optionChips: { marginTop: spacing.sm, flexDirection: 'row', flexWrap: 'wrap', gap: 7 }, optionChip: { minHeight: 32, paddingHorizontal: 10, flexDirection: 'row', gap: 5, alignItems: 'center', borderRadius: 16, backgroundColor: colors.lifeLight }, optionText: { color: colors.life, fontSize: 9 }, optionCreate: { marginTop: spacing.md, flexDirection: 'row', borderTopWidth: StyleSheet.hairlineWidth, borderTopColor: colors.line }, optionInput: { flex: 1, minHeight: 46, paddingHorizontal: spacing.sm, color: colors.ink, fontSize: 11 }, optionAdd: { width: 44, height: 44, marginTop: 2, alignItems: 'center', justifyContent: 'center' },
  backdrop: { flex: 1, justifyContent: 'flex-end', backgroundColor: colors.backdrop }, sheet: { padding: spacing.lg, paddingBottom: spacing.xxl, borderTopLeftRadius: radius.xl, borderTopRightRadius: radius.xl, backgroundColor: colors.paper }, handle: { width: 36, height: 4, marginBottom: spacing.lg, alignSelf: 'center', borderRadius: 2, backgroundColor: colors.line }, sheetTitle: { color: colors.ink, fontFamily: typography.display, fontSize: 22 }, editInput: { minHeight: 52, marginTop: spacing.lg, paddingHorizontal: spacing.md, borderRadius: 6, backgroundColor: colors.sheet, color: colors.ink }, confirmButton: { minHeight: 52, marginTop: spacing.md, alignItems: 'center', justifyContent: 'center', borderRadius: 6, backgroundColor: colors.life }, confirmText: { color: colors.onLife, fontSize: 11, fontWeight: '700' },
}));
