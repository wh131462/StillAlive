...
import { persistAlbumImage } from '../../infrastructure/files/local-media';
...
export default function AlbumScreen() {
  ...
  const importPhotos = async (assets: ImagePickerAsset[]) => {
    if (!album) return;
    setImporting(true);
    let failed = 0;
    ...
    setImporting(false);
    if (failed) feedback.alert('部分照片未添加', `${failed} 张照片处理失败，已清理未完成文件。`);
  };
  ...
  const takePhoto = async () => {
    ...
    if (!result.canceled) await importPhotos(result.assets);
  };
  ...
  const confirmDelete = () => {
    if (!album) return;
    feedback.alert(`删除相册“${album.name}”？`, `将永久删除应用内的 ${photos.length} 张照片，不影响系统照片库原图。`, [{ text: '取消', style: 'cancel' }, { text: '删除相册', style: 'destructive', onPress: () => void deleteAlbum(album.id).then(() => router.back()) }]);
  };
...
function AlbumPhoto({ uri }: { uri: string }) {
  ...
  return failed ? <View style={styles.brokenPhoto}><SymbolView name={{ android: 'broken_image', ios: 'photo.badge.exclamationmark', web: 'broken_image' }} size={24} tintColor={colors.inkFaint} type="hierarchical" /><Text style={styles.brokenText}>图片不可用</Text></View> : <Image onError={() => setFailed(true)} resizeMode="cover" source={{ uri }} style={styles.photo} />;
}
...
const styles = createThemedStyles(() => ({
  flex: { flex: 1 }, safeArea: { flex: 1, backgroundColor: colors.paper }, header: { minHeight: 64, paddingHorizontal: spacing.md, flexDirection: 'row', alignItems: 'center', borderBottomWidth: StyleSheet.hairlineWidth, borderBottomColor: colors.line }, headerSide: { width: 88 }, headerButton: { width: 44, height: 44, alignItems: 'center', justifyContent: 'center', borderRadius: 22 }, headerButtonPressed: { backgroundColor: colors.lifeLight }, headerActions: { width: 88, flexDirection: 'row' }, headerCopy: { flex: 1, alignItems: 'center' }, headerTitle: { color: colors.ink, fontFamily: typography.display, fontSize: 18 }, headerMeta: { marginTop: 3, color: colors.inkFaint, fontSize: 8 }, content: { padding: spacing.md, paddingBottom: spacing.xxl },
