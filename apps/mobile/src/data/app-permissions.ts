...
export async function ensureAppPermission(permission: AppPermission, extraActions: AlertButton[] = []): Promise<boolean> {
  ...
  try {
    ...
    Alert.alert('权限检查失败', cause instanceof Error ? cause.message : '请稍后重试。');
    return false;
  }
